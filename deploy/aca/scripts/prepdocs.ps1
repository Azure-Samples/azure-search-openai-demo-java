...
Start-Process -FilePath "java" -ArgumentList "-jar ../../app/indexer/cli/target/cli.jar `"../../data`" --storageaccount $env:AZURE_STORAGE_ACCOUNT --container $env:AZURE_STORAGE_CONTAINER --searchservice $env:AZURE_SEARCH_SERVICE --openai-service-name $env:AZURE_OPENAI_SERVICE --openai-emb-deployment $env:AZURE_OPENAI_EMB_DEPLOYMENT --index $env:AZURE_SEARCH_INDEX --document-intelligent-service $env:AZURE_DOCUMENT_INTELLIGENCE_SERVICE --embedding-dimensions $env:AZURE_OPENAI_EMB_DIMENSIONS --indexer-url $env:INDEXER_URI --verbose upload" -Wait -NoNewWindow