...
var omsAgentConfig = (!empty(logAnalyticsWorkspaceName) && !empty(addOns.omsAgent) && addOns.omsAgent.enabled) ? union(
  addOns.omsAgent,
  {
    config: {
...
resource logAnalytics 'Microsoft.OperationalInsights/workspaces@2021-12-01-preview' existing = if (!empty(logAnalyticsWorkspaceName)) {
...
module managedCluster 'aks-managed-cluster.bicep' = {
  ...
  params: {
    ...
    addOns: addOnsConfig
    workspaceId: !empty(logAnalyticsWorkspaceName) ? logAnalytics.id : ''
  }
}
