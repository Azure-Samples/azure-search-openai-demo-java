...
public class AskAPITests {

	@Autowired
	private TestRestTemplate restTemplate;
	@MockBean
	private CognitiveSearchProxy cognitiveSearchProxyMock;
	...
	public void testExample() throws Exception {
		...
		askRequest.setQuestion("What does a Product Manager do?");
		askRequest.setApproach("rtr");

		HttpEntity<AskRequest> request = new HttpEntity<>(askRequest);

		ResponseEntity<AskResponse> result = this.restTemplate.postForEntity(uri("/api/ask"), request, AskResponse.class);

		assertEquals(HttpStatus.OK, result.getStatusCode());
		assertEquals("Product managers put items in roadmaps and backlogs", result.getBody().getAnswer());
		assertEquals(2,result.getBody().getDataPoints().size());
	...
	private void prepareMocks() {
		...
		when(cognitiveSearchProxyMock.search(eq("What does a Product Manager do?"),any(SearchOptions.class),eq(Context.NONE))).thenReturn(searchPagedIterable);

		Completions mockedCompletions = buildCompletions();

		// CompletionsOptions doesn't implement equals so we can't use the when().thenReturn() pattern
		/**
		var completionsOptions = buildCompletionsOptionsWithMockData();
		when(openAIProxyMock.getCompletions(eq(completionsOptions))).thenReturn(mockedCompletions);
		 */

		when(openAIProxyMock.getCompletions(any(CompletionsOptions.class))).thenReturn(mockedCompletions);
	}

	private Completions buildCompletions() {
		OpenAIUnitTestUtils utils = new OpenAIUnitTestUtils();
		Choice choice1 = utils.createChoice("Product managers put items in roadmaps and backlogs",0);
	...
	private SearchPagedIterable buildSearchPagedIterableWithDocs() {
		...
		return  new SearchPagedIterable(utils.getSearchPagedFlux(1,(inputInteger) -> {return List.of(cit1SearchResult,cit2SearchResult);}));
	}
	...
	private CompletionsOptions buildCompletionsOptionsWithMockData() {
		CompletionsOptions completionsOptions = new CompletionsOptions(new ArrayList<>( Arrays.asList(promptWithMockedSources)));
	...
	final private String promptWithMockedSources = """
